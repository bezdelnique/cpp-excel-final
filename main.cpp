...
int main() {
  ...
  {
    Position pos{0, 0};
    assert(pos.ToString() == "A1");
    Position pos0 = Position::FromString("A1");
    assert(pos == pos0);
  }

  // AZ1
  {
    Position pos{0, 26 + 26 - 1};
    assert(pos.ToString() == "AZ1");
    Position pos0 = Position::FromString("AZ1");
    assert(pos == pos0);
  }

  {
    ...
    int col = (26 + 1) - 1;
    Position pos{0, col};
    assert(pos.ToString() == "AA1");
    Position pos0 = Position::FromString("AA1");
    assert(pos == pos0);
  }

  {
    //XFD16384
    Position pos{Position::MAX_ROWS - 1, Position::MAX_COLS - 1};
    assert(pos.ToString() == "XFD16384");
    Position pos0 = Position::FromString("XFD16384");
    assert(pos == pos0);
  }

  {
    //AAA1
    int col = ((26 * 26) * 1) + (26 * 1) + 1;
    Position pos{0, col - 1};
    assert(pos.ToString() == "AAA1");
    Position pos0 = Position::FromString("AAA1");
    assert(pos == pos0);
  }
